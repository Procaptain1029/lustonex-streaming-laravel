...
@section('title', __('fan.notifications.title'))
...
<div class="notifications-container">
    <div class="page-header">
        <h1 class="page-title">
            <i class="fas fa-bell"></i> {{ __('fan.notifications.header') }}
        </h1>
        @if($stats['unread'] > 0)
        <button class="btn-mark-all" onclick="markAllAsRead()">
            <i class="fas fa-check-double"></i> {{ __('fan.notifications.mark_all_read') }}
        </button>
        @endif
    </div>
    ...
    <div class="stats-row">
        <div class="stat-card">
            ...
            <span class="stat-label">{{ __('fan.notifications.stats.total') }}</span>
        </div>
        <div class="stat-card">
            <span class="stat-value" style="color: var(--color-oro-sensual, #D4AF37);">{{ number_format($stats['unread']) }}</span>
            <span class="stat-label">{{ __('fan.notifications.stats.unread') }}</span>
        </div>
        <div class="stat-card">
            <span class="stat-value">{{ number_format($stats['today']) }}</span>
            <span class="stat-label">{{ __('fan.notifications.stats.today') }}</span>
        </div>
    </div>
    ...
    <div class="notifications-card">
        <div class="notifications-list">
            ...
            <div class="notification-item {{ $read ? 'read' : 'unread' }}" data-id="{{ $notification->id }}">
                ...
                <div class="notification-content">
                    <div class="notification-title">
                        <span>{{ $data['title'] ?? __('fan.notifications.default_title') }}</span>
                ...
                <div class="notification-actions">
                    @if(!$read)
                    <button class="btn-action-sm" onclick="markAsRead('{{ $notification->id }}', event)" title="{{ __('fan.notifications.mark_as_read') }}">
            ...
            <div class="empty-state">
                ...
                <h3>{{ __('fan.notifications.empty_title') }}</h3>
                <p>{{ __('fan.notifications.empty_subtitle') }}</p>
            </div>
            @endforelse
        </div>
    </div>
...
function markAsRead(id, event) {
    ...
    fetch(`/fan/notifications/${id}/read`, {
        method: 'POST',
        headers: {
...
function markAllAsRead() {
    Swal.fire({
        title: '{{ __('fan.notifications.swal.title') }}',
        text: "{{ __('fan.notifications.swal.text') }}",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#D4AF37',
        cancelButtonColor: 'rgba(255,255,255,0.1)',
        confirmButtonText: '{{ __('fan.notifications.swal.confirm') }}',
        cancelButtonText: '{{ __('fan.notifications.swal.cancel') }}',
        background: '#1a1a1f',
        color: '#fff',
        customClass: {
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        title: '{{ __('fan.notifications.swal.success_title') }}',
                        text: '{{ __('fan.notifications.swal.success_text') }}',
                        icon: 'success',
                        background: '#1a1a1f',
                        color: '#fff',
                        confirmButtonColor: '#D4AF37',
            ...
            .catch(error => {
                ...
                Swal.fire({
                    title: 'Error',
                    text: '{{ __('fan.notifications.swal.error_text') }}',
                    icon: 'error',
                    background: '#1a1a1f',
                    color: '#fff',
                    confirmButtonColor: '#D4AF37'
