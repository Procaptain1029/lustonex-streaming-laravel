...
@section('title', __('admin.token_packages.edit_title'))
...
@section('breadcrumb')
    ...
    <span class="breadcrumb-separator"><i class="fas fa-chevron-right"></i></span>
    <span class="breadcrumb-item active">{{ __('admin.token_packages.edit_title') }}</span>
@endsection
...
@section('content')
    <div class="page-header">
        <h1 class="page-title">{{ __('admin.token_packages.edit_header') }}</h1>
        <p class="page-subtitle">{{ __('admin.token_packages.edit_subtitle') }} <strong style="color:var(--admin-gold)">{{ $tokenPackage->name }}</strong></p>
    </div>
    ...
    <div class="sh-page-grid">
        ...
        <div class="sh-form-column">
            <div class="sh-form-card">
                ...
                <form action="{{ route('admin.token-packages.update', $tokenPackage) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="sh-form-section">
                        ...
                        <div class="sh-form-group">
                            ...
                            <input type="text" name="name" class="sh-input" value="{{ old('name', $tokenPackage->name) }}" required>
                        </div>
                        ...
                        <div class="sh-reward-grid">
                            <div class="sh-form-group">
                                ...
                                <div style="position: relative;">
                                    <input type="number" name="tokens" class="sh-input" value="{{ old('tokens', $tokenPackage->tokens) }}"
                            ...
                            <div class="sh-form-group">
                                ...
                                <div style="position: relative;">
                                    <input type="number" name="bonus" class="sh-input" value="{{ old('bonus', $tokenPackage->bonus) }}"
                        ...
                        <div class="sh-form-group">
                            ...
                            <div class="sh-input-group">
                                ...
                                <input type="number" step="0.01" name="price" class="sh-input" value="{{ old('price', $tokenPackage->price) }}" required min="0.01">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sh-form-section">
                        ...
                        <div class="sh-reward-grid">
                            <div class="sh-form-group" style="display: flex; align-items: flex-end; padding-bottom: 22px;">
                                <label class="sh-toggle">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $tokenPackage->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="sh-form-group" style="display: flex; align-items: flex-end; padding-bottom: 22px;">
                                <label class="sh-toggle">
                                    <input type="checkbox" id="is_limited_time" name="is_limited_time" value="1" {{ old('is_limited_time', $tokenPackage->is_limited_time) ? 'checked' : '' }} onchange="toggleExpiresAt()">
                                    <div class="sh-toggle-track">
                                        <div class="sh-toggle-thumb"></div>
                                    </div>
                                    <div>
                        ...
                        <div class="sh-form-group" id="expires_at_container" style="{{ old('is_limited_time', $tokenPackage->is_limited_time) ? '' : 'display: none;' }}">
                            <label class="sh-label">{{ __('admin.token_packages.form.expires_label') }}</label>
                            <input type="hidden" id="expires_at_utc" value="{{ $tokenPackage->expires_at ? $tokenPackage->expires_at->format('Y-m-d\TH:i:s\Z') : '' }}">
                    ...
                    <div class="sh-form-actions">
                        ...
                        <button type="submit" class="sh-btn sh-btn-primary">
                            <i class="fas fa-save"></i> {{ __('admin.token_packages.form.update_btn') }}
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Convert UTC to local time for the date input
        const expiresInput = document.getElementById('expires_at');
        if (!expiresInput.value) {
            const utcStr = document.getElementById('expires_at_utc').value;
            if (utcStr) {
